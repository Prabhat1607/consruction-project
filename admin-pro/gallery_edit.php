<?php
    ...
    $sql_edit = "SELECT gt.*,st.service_type FROM gallery_tbl gt LEFT JOIN service_tbl st ON gt.service_id = st.service_id WHERE gt.gallery_id = '".$_GET['id']."' ORDER BY gt.gallery_id desc";
                            ...
                            <div class="row p-t-20">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control custom-select" name="cmb_type" id="cmb_type">
                                            ...
                                            <?php
                                                ...
                                                {
                                                    if ($row_edit['service_id'] == $row_service['service_id']) 
                                                    { ?>
                                                        <option value="<?php echo $row_service['service_id']; ?>"
                                                            selected="selected">
                                                            <?php echo $row_service['service_type'] ?></option>
                                                        <?php
                                                    } 
                                                        else 
                                                    { ?>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group has-danger">
                                        ...
                                        <img src="photos/service-image/<?php if ($row_edit['gallery'] == "") 
                                        {
                                            echo 'user.png';
                                        } else {
                                            echo $row_edit['gallery'];
                                        } ?>"
...
<?php
    ...
    if(isset($_POST['btn_gallery']))
    {
        ...
        {
            $filename = 'photos/service-image/' . $row_edit['gallery'];
            unlink($filename);
        ...
        {
            $img = $row_edit ['gallery'];
        }
      $sql = "UPDATE gallery_tbl SET  service_id = '".$_POST['cmb_type']."',gallery = '".$img."'
      WHERE gallery_id = '".$_GET['id']."'";
      $rs = mysqli_query($con,$sql);
      if(!$rs)
      {
        die('No Updated.'.mysqli_error($con));
      }
